...
export function LiquidGlassSlider({
  ...
  const thumbIdleSurface = useGlassSurface({ variant: "thumb" });
  const thumbActiveSurface = useGlassSurface({ variant: "thumb-active" });
  const thumbSurface = isDragging ? thumbActiveSurface : thumbIdleSurface;
  
  // Track fill surface
  const trackFillSurface = useGlassSurface({ variant: "track-active", tint: accentColor, activeTint: accentColor });

  // Sync localValue when prop changes from outside (and not actively dragging)
  useEffect(() => {
    if (!isDragging) {
      setLocalValue(value);
    }
